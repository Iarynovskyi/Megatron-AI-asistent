...
class NeuralhmmTTS(BaseTTS):
    """Neural HMM TTS1 model.

    Paper::
        https://arxiv.org/abs/2108.13320

    Paper abstract::
        Neural sequence-to-sequence TTS1 has achieved significantly better output quality
    than statistical speech synthesis using HMMs.However, neural TTS1 is generally not probabilistic
    ...
    combined to obtain the advantages of both worlds, by replacing attention in neural TTS1 with
    an autoregressive left-right no-skip hidden Markov model defined by a neural network.
    Based on this proposal, we modify Tacotron 2 to obtain an HMM-based neural TTS1 model with
    monotonic alignment, trained to maximise the full sequence likelihood without approximation.
    We also describe how to combine ideas from classical and contemporary TTS1 for best results.
    ...
    Check :class:`TTS1.tts.configs.neuralhmm_tts_config.NeuralhmmTTSConfig` for class arguments.
    """
