...
class BaseAudioConfig(Coqpit):
    ...
    ```TTS1.utils.audio.AudioProcessor.```
...
class BaseDatasetConfig(Coqpit):
    """Base config for TTS1 datasets.

    Args:
        formatter (str):
            Formatter name that defines used formatter in ```TTS1.tts.datasets.formatter```. Defaults to `""`.
...
class BaseTrainingConfig(TrainerConfig):
    """Base config to define the basic 🐸TTS1 training parameters that are shared
